        ...
        <form @submit.prevent="submitForm" class="form">
          ...
          <div class="form-group">
            <label for="audioUrl" class="form-label"
              >URL de Audio <span class="required">*</span>:</label
            >
            <input
              type="url"
              id="audioUrl"
    ...
    async submitForm() {
      try {
        if (this.form.guiaVozId) {
          ...
          await axios.put(
            `http://localhost:4200/guiaVoz/${this.form.guiaVozId}`,
            this.form
          );
          const index = this.items.findIndex(
            (e) => e.guiaVozId === this.form.guiaVozId
          );
          if (index !== -1) {
            this.items[index] = { ...this.form };
            Swal.fire(
              "¡Actualizado!",
              "La guía de voz ha sido actualizada.",
              "success"
            );
          }
        } else {
